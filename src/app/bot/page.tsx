...
import { 
  ...
  Target, BrainCircuit, Activity, Lock, Coins, Archive
} from "lucide-react";
...
export default function SovereignDecisionBot() {
  ...
  // Monitor user for moderation & balance with safety
  useEffect(() => {
    if (!db || !user) return;
    const unsub = onSnapshot(doc(db, "users", user.uid), (doc) => {
      ...
      console.warn("Moderation stream failed, proceeding with safety limited mode.");
    });
    return () => unsub();
  }, [db, user, toast]);
      ...
      <main className="flex-1 flex flex-col relative bg-black">
                ...
                <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="space-y-10 pb-40">
                   <Card className="glass-cosmic border-none rounded-[4rem] p-14 shadow-2xl relative overflow-hidden">
                      <div className="flex justify-between items-start mb-16">
                         ...
                         <Badge className={`py-2 px-8 rounded-full font-black text-lg border-none shadow-2xl ${decisionData?.riskLevel === 'critical' ? 'bg-red-600' : 'bg-emerald-600'}`}>
                           RISK: {decisionData?.riskLevel.toUpperCase()}
                         </Badge>
                      </div>
                      <div className="grid md:grid-cols-2 gap-12 text-right">
                         <div className="space-y-4">
                            <h4 className="text-sm font-black text-primary uppercase tracking-[0.2em]">الإجراء الفوري</h4>
                            <p className="text-xl font-bold text-white/90">{decisionData?.recommendedAction}</p>
                         </div>
                         <div className="space-y-4">
                            <h4 className="text-sm font-black text-white/20 uppercase tracking-[0.2em]">التنبؤ الاستراتيجي</h4>
                      ...
                      <div className="mt-16 pt-10 border-t border-white/5 flex justify-between items-center">
                         ...
                         <div className="flex gap-4">
                            <Button className="btn-primary h-14 px-10 rounded-2xl font-black gap-3"><Archive className="h-4 w-4" /> حفظ</Button>
                         </div>
                      </div>
                   </Card>
                </motion.div>
        ...
        <div className="absolute bottom-0 inset-x-0 px-8 pb-12 pt-10 bg-gradient-to-t from-black via-black/90 to-transparent z-30">
           <div className="max-w-4xl mx-auto">
              <div className="relative glass-cosmic rounded-[2.5rem] p-3 flex items-center gap-4 shadow-2xl border-white/10">
                 <textarea 
                   rows={1}
                 ...
                 <Button 
                   ...
                   className="h-14 w-14 rounded-2xl bg-white text-black hover:bg-indigo-50 shadow-2xl"
                 >
