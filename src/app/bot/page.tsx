    ...
    <ProtectedRoute>
      <div className="flex h-screen bg-[#02020a] overflow-hidden font-sans" dir="rtl">
        <AnimatePresence mode="wait">
          {isSidebarOpen && (
            <motion.aside initial={{ width: 0, opacity: 0 }} animate={{ width: 320, opacity: 1 }} exit={{ width: 0, opacity: 0 }} className="hidden lg:flex flex-col flex-shrink-0 z-50">
              <SidebarContent />
            </motion.aside>
          )}
        </AnimatePresence>
          ...
          <main className="flex-1 flex flex-col relative overflow-hidden bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-primary/5 via-transparent to-transparent">
              ...
              <div className="max-w-4xl mx-auto space-y-8">
                {cloudMessages?.map((m) => (
                  <div key={m.id} className={`flex flex-col ${m.role === 'user' ? 'items-start' : 'items-end'}`}>
                    <div className={`p-6 rounded-[2.5rem] max-w-[85%] text-lg leading-relaxed shadow-2xl border ${
                      m.role === 'user' ? 'bg-white text-slate-900 font-bold rounded-tr-none' : 'bg-[#0a0a1f] text-white border-primary/20 rounded-tl-none font-medium'
                    }`}>
                      {m.text}
                    </div>
                  </div>
            ...
            <div className="absolute bottom-0 inset-x-0 p-8 z-20">
              <div className="max-w-4xl mx-auto space-y-4">
                <div className="flex items-center gap-3">
                   <ToolBtn icon={<Paperclip />} onClick={() => fileInputRef.current?.click()} />
                   <ToolBtn icon={<Camera />} onClick={() => setIsCameraOpen(true)} />
                   <ToolBtn icon={<Mic />} active={isRecording} color="red" />
                   <input type="file" ref={fileInputRef} className="hidden" multiple />
                </div>
                <div className="relative glass-cosmic border-2 border-white/10 rounded-[2.5rem] overflow-hidden shadow-3xl">
...
function CommandBtn({ icon, text, onClick, color }: any) {
  ...
  return (
    <button onClick={onClick} className={`w-full flex items-center justify-between px-6 py-4 rounded-2xl border transition-all ${colors[color]}`}>
...
function ToolBtn({ icon, onClick, active, color }: any) {
  return (
    <button onClick={onClick} className={`h-12 w-12 rounded-xl flex items-center justify-center transition-all border shadow-lg ${active ? 'bg-red-500 border-red-500 text-white animate-pulse' : 'bg-white/5 border-white/5 text-white/40 hover:text-primary hover:border-primary/20'}`}>
      {icon}
    </button>
  );
}
